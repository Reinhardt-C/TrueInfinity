class Upgrade {
	...
	get level() {
		if (!game[this.loc == "upgrades" ? "upgradesBought" : "unfunityUpgBought"][this.id])
			game[this.loc == "upgrades" ? "upgradesBought" : "unfunityUpgBought"][this.id] = D(0);
	...
	get afford() {
		return (
			this.currency &&
			this.currency.gte(this.cost) &&
			(this.levelCap.eq(0) || this.level.lt(this.levelCap))
		);
	}

	get cost() {
		return this.baseCost
			.times(D.pow(this.costScale, this.level))
			.times(D.pow(this.CSI, D.sumArithmeticSeries(this.level, D(1), D(1), D(0))));
	}
	...
	domCreate() {
		...
		div.id = "upg" + this.id;
		div.className = "upg";
		div.innerHTML = `<span id='upgn${this.id}'>${this.name}</span>`;
	...
	domUpdate() {
		...
		document.getElementById("buyupg" + this.id).innerText = this.level.eq(this.levelCap)
			? "MAXED"
			: `Buy for ${f(this.cost) + " " + this.currencyName}`;
		document.getElementById("tupg" + this.id).innerText = `
      Level: ${this.level}
	`;

		if (this.level.eq(this.levelCap))
			document.getElementById("buyupg" + this.id).className = document
				.getElementById("buyupg" + this.id)
				.className.replace(/(green|red)/, "blue");
		else {
			if (this.afford)
				document.getElementById("buyupg" + this.id).className = document
					.getElementById("buyupg" + this.id)
					.className.replace(/(blue|red)/, "green");
			else
				document.getElementById("buyupg" + this.id).className = document
					.getElementById("buyupg" + this.id)
					.className.replace(/(green|blue)/, "red");
		}
	}
	...
	buy(aut = false) {
		...
		if (!(game.axioms[3] && this.loc == "unfunityUpgrades")) this.currency.subBy(this.cost);
		this.level.addBy(1);
		if (this.id == "dimComp" && game.milestoneUnlocked(3)) return true;
...
function setupUpgrades() {
	...
	for (let i in config.unfunityUpgrades)
		game.unfunityUpgrades[i] = new Upgrade(i, "unfunityUpgrades");
}
