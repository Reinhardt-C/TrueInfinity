...
window.addEventListener("error", function (event) {
	...
	var id = "";
	for (var i = 0; i < 6; i++, idnum /= 62)
		id += "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"[idnum % 62];
	n.id = "err_" + id;
	...
	m.style.float = "left";
	m.onclick = new Function(
		'var e=document.getElementById("err_' +
			id +
			'_c");e.style.display="";e.select();e.setSelectionRange(0,99999);document.execCommand("copy");e.style.display="none";e=null;'
	);
	n.appendChild(m);
	...
	m.onclick = new Function(
		'document.getElementById("errC").removeChild(document.getElementById("err_' +
			id +
			'"));if (document.getElementById("errC").childNodes.length===0) document.getElementById("err").style.display="none";'
	);
	n.appendChild(m);
